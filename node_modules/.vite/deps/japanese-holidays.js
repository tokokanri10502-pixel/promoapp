...
//#region node_modules/japanese-holidays/lib/japanese-holidays.js
var require_japanese_holidays$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		"use strict";
		var definition, furikaeHoliday, getHolidaysOf, getJDate, getJDay, getJFullYear, getJHours, getJMinutes, getJMonth, happyMonday, holidays, isHoliday, isHolidayAt, j2u, jDate, kokuminHoliday, ref, shiftDate = function(date, year, mon, day, hour, min, sec, msec) {
			var res = new Date(2e3, 0, 1);
			res.setTime(date.getTime() + ((((day != null ? day : 0) * 24 + (hour != null ? hour : 0)) * 60 + (min != null ? min : 0)) * 60 + (sec != null ? sec : 0)) * 1e3 + (msec != null ? msec : 0));
			...
			return res;
		}, shubun, shubunWithTime, shunbun, shunbunWithTime, simpleHoliday, target, u2j = function(d) {
		...
		shunbunWithTime = function(year) {
			return /* @__PURE__ */ new Date(-6558667e5 + 31556940400 * (year - 1949));
		};
		shunbun = function(year) {
			var date = shunbunWithTime(year);
		...
		shubunWithTime = function(year) {
			...
			if (day = {
				...
				2355: 23,
				2384: 22
			}[year]) return jDate(year, 8, day);
			else return /* @__PURE__ */ new Date(-67131691e4 + 3155691e4 * (year - 1948));
		};
		shubun = function(year) {
			var date = shubunWithTime(year);
			return jDate(year, getJMonth(date), getJDate(date));
		};
		definition = [
			[
		...
		furikaeHoliday = function(holiday) {
			var furikae;
			if (holiday < jDate(1973, 3, 29) || getJDay(holiday) !== 0) return null;
			furikae = shiftDate(holiday, 0, 0, 1);
			if (!isHolidayAt(furikae, false)) return furikae;
			if (holiday < jDate(2007, 0, 1)) return null;
			while (true) {
		...
		kokuminHoliday = function(holiday) {
			...
			if (isHolidayAt(kokumin, false) || getJDay(kokumin) === sunday || getJDay(kokumin) === monday) return null;
			return kokumin;
		};
		holidays = {
			"true": {},
		...
		getHolidaysOf = function(y, furikae) {
			...
			for (i = 0, len = definition.length; i < len; i++) {
				entry = definition[i];
				if (entry[2] != null && y < entry[2]) continue;
...
//#endregion
//#region node_modules/japanese-holidays/index.js
var require_japanese_holidays = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = require_japanese_holidays$1();
}));
//#endregion
export default require_japanese_holidays();
